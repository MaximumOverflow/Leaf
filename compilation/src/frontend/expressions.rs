...
pub fn compile_expression(
	block: &Block, expr: &Expression, builder: &mut FunctionBodyBuilder,
) -> anyhow::Result<Value> {
	match expr {
		Expression::Literal(literal) => match literal {
			...
			Literal::Id(id) => match block.values().get(*id) {
				Some(value) => Ok(value.clone()),
				None => Err(anyhow::Error::msg(format!(
					"Identifier {:?} not found in the current scope",
					id
				))),
			},
			...
			match ty.variant() {
				TypeVariant::Struct(s_ty) => {
					...
					for (name, (_, value)) in fields {
						let Some(field_idx) =
							s_ty.fields().iter().position(|f| &**f.name() == *name)
						else {
...
impl Value {
	pub fn ty(&self) -> Option<&Arc<Type>> {
		match self {
			| Value::Temp(t) | Value::Const(t) | Value::Local(t, ..) | Value::Param(t, ..) => {
				Some(t)
			},
			_ => None,
		}
	...
	pub fn load(&self, opcodes: &mut FunctionBodyBuilder) -> anyhow::Result<()> {
		match self {
			...
			Value::Param(_, i, ..) => Ok(opcodes.push_param_address(*i).unwrap()),
			_ => Ok(()),
		}
	}
}
