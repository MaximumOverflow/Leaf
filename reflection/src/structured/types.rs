	...
	pub fn make_ptr(&self, mutable: bool) -> &Arc<Type> {
		match mutable {
			false => self.ptr_ty.get_or_init(|| {
				Type::new(
					TypeVariant::Pointer(self.this.clone(), false),
					ElementRef::default(),
				)
			}),
			true => self.mut_ptr_ty.get_or_init(|| {
				Type::new(
					TypeVariant::Pointer(self.this.clone(), true),
					ElementRef::default(),
				)
			}),
		}
...
impl Field {
	...
	pub fn ty(&self) -> Arc<Type> {
		self.ty
			.upgrade()
			.expect("The field's type has been dropped and is no longer available")
	}
}
...
impl StructBuilder {
	pub(crate) fn new(
		name: &str, namespace: Arc<str>, type_def: ElementRef<TypeDef>,
	) -> StructBuilder {
		Self {
			ty: Type::new(
				TypeVariant::Struct(StructType {
					namespace,
