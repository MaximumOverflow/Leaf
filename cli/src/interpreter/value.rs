...
impl Value {
	...
	pub unsafe fn init<T>(
		&mut self, func: impl FnOnce(&mut [u8]) -> anyhow::Result<T>,
	) -> anyhow::Result<T> {
		match self.ty.variant() {
			TypeVariant::Void => func(&mut []),
			TypeVariant::Bool => {
				let Data::Bool(i) = &mut self.data else {
					unreachable!()
				};
				let mut v = 0i8;
			...
			TypeVariant::Char => {
				let Data::Char(ch) = &mut self.data else {
					unreachable!()
				};
				let mut v = 0u32;
				let res = func(bytemuck::bytes_of_mut(&mut v))?;
			...
			TypeVariant::Int8 => {
				let Data::Int(i) = &mut self.data else {
					unreachable!()
				};
				let mut v = 0i8;
				let res = func(bytemuck::bytes_of_mut(&mut v))?;
			...
			TypeVariant::UInt8 => {
				let Data::UInt(i) = &mut self.data else {
					unreachable!()
				};
				let mut v = 0u8;
				let res = func(bytemuck::bytes_of_mut(&mut v))?;
			...
			TypeVariant::Float16 => {
				let Data::Float(f) = &mut self.data else {
					unreachable!()
				};
			...
			_ => match &self.data {
				...
				_ => Err(anyhow!(
					"Value is already initialized or should contain no data"
				)),
			},
		}
	}
